...
### Fixed
- **wrong audio paired with right metadata on some tracks** (closes #52). reporter showed an mp3 named `Mi Gente - DJ Goja.mp3` with DJ Goja's cover art, but the audio was actually `Mi Chico` (different song, same artist). root cause: the YouTube top hit for `ytsearch5:Mi Gente DJ Goja audio` was `Dj Goja - Mi Chico` at 117s — within ±7s of Spotify's real Mi Gente (114.8s), so the v2.0.7 duration-only matcher trusted it and wrote Mi Gente metadata onto Mi Chico audio. selection now requires BOTH the song title AND at least one Spotify artist to appear in the YouTube title, AND the duration to be within ±30s. when no candidate satisfies all three, the matcher returns None and the user gets a clear `not found on YouTube` error - strictly better than wrong audio under right cover. handles edge cases that were also dropping: spotify `Title - Remastered 2011` suffix stripped before matching, multi-artist collaborations (`Luis Fonsi, Daddy Yankee`) match if ANY artist appears, accented and apostrophe-bearing titles normalize correctly (`Café` -> `cafe`, `I'm Yours` -> `im yours` not `i m yours`), youtube's `Artist - Song` format is NOT split (the variant strip applies to spotify-side titles only). 11 new tests cover the bug case + every regression-prone shape.
